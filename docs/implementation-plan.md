...
| Layer | Platform | Repo path | Notes |
|-------|----------|-----------|-------|
| **Frontend** | Vercel | `frontend/` | Static SPA; `npm run build` → `dist/` |
| **Backend** | Railway | `backend/` | Docker or Nixpacks; root `backend/Dockerfile` |
| **Database** | Railway | — | Prefer **PostgreSQL** add-on; SQLite OK for early deploy |

**Cross-service env wiring:**

| Variable | Where | Value |
|----------|-------|-------|
| `VITE_API_URL` | Vercel | `https://<railway-api-domain>` |
| `CORS_ORIGINS` | Railway | `https://<vercel-app>.vercel.app` (+ preview URLs if needed) |
| `DATABASE_URL` | Railway | Postgres connection string from Railway plugin |
| `GROQ_API_KEY` | Railway | Secret; never exposed to frontend |

---

### Milestone 3.1 — Multi-Format Parsing

**Target:** Days 1–2

| # | Task | Files / Modules | Depends On |
|---|------|-----------------|------------|
| 3.1.1 | Second bank parser (ICICI or SBI) | `parsers/icici.py` or `parsers/sbi.py` | 1.2.1 |
| 3.1.2 | Generic CSV column mapper (fallback) | Detect date, description, amount, debit/credit columns | 3.1.1 |
| 3.1.3 | Excel support (.xlsx) | `openpyxl` via pandas | 3.1.2 |
| 3.1.4 | Sample CSV template | `docs/sample-statement-template.csv` for users | — |
| 3.1.5 | Upload UI format hints | List supported banks + link to template | 1.6.2 |

**Acceptance criteria:**
- [ ] At least 2 bank CSV formats parse correctly
- [ ] Generic CSV works with standard column headers
- [ ] Clear error + template link for unsupported files

**Tests:**
- [ ] Fixture per supported bank format
- [ ] Golden file outputs comparable structure

---

### Milestone 3.2 — Report Export

**Target:** Days 2–3

| # | Task | Files / Modules | Depends On |
|---|------|-----------------|------------|
| 3.2.1 | HTML report template | Summary, charts as static data, top insights, recurring list | Phase 2 |
| 3.2.2 | `GET /api/v1/sessions/{id}/report` | Return HTML or `?format=pdf` | 3.2.1 |
| 3.2.3 | PDF generation | `weasyprint` or browser print-to-PDF | 3.2.2 |
| 3.2.4 | `ReportExport` component | Download PDF / Print button | 3.2.2 |
| 3.2.5 | Shareable single-page layout | Screenshot-friendly summary page | 3.2.1 |

**Acceptance criteria:**
- [ ] User downloads PDF or prints report with summary, categories, insights
- [ ] Report uses actual amounts (no placeholder text)
- [ ] Report readable without the web app open

---

### Milestone 3.3 — Privacy & Session Lifecycle

**Target:** Day 3

| # | Task | Files / Modules | Depends On |
|---|------|-----------------|------------|
| 3.3.1 | `DELETE /api/v1/sessions/{id}` | Cascade delete transactions, groups, analysis | 1.1.1 |
| 3.3.2 | Session TTL | `expires_at` on create; background purge job or check on read | 1.1.1 |
| 3.3.3 | `SESSION_TTL_HOURS` env | Default 72 hours | 3.3.2 |
| 3.3.4 | Privacy notice in UI | Short copy: data retention, no third-party storage of files | 1.6.2 |
| 3.3.5 | Audit logging hygiene | Ensure no raw balances/account numbers in logs | Phase 1 |

**Acceptance criteria:**
- [ ] DELETE removes all session data
- [ ] Expired sessions return 404 or auto-purged
- [ ] Raw upload files never persist after parse

---

### Milestone 3.4 — LLM-Enhanced Narrative Insights

**Target:** Day 4

| # | Task | Files / Modules | Depends On |
|---|------|-----------------|------------|
| 3.4.1 | Narrative insight prompt | Feed metrics + top categories + recurring summary only | 2.1.1 |
| 3.4.2 | Merge with template insights | Templates first; LLM adds 2–3 observations | 1.5.3 |
| 3.4.3 | Fallback | Templates only if LLM unavailable | 3.4.2 |
| 3.4.4 | Display in `InsightCards` | Distinguish template vs. AI-generated (optional badge) | 1.6.7 |

**Acceptance criteria:**
- [ ] ≥ 5 total insights when LLM enabled (3 template + 2 narrative)
- [ ] Narrative insights reference real amounts and categories
- [ ] No PII in LLM payload

---

### Milestone 3.5 — Deployment (Railway + Vercel)

**Target:** Days 4–5

#### 3.5.1 — Backend on Railway

| # | Task | Files / Modules | Depends On |
|---|------|-----------------|------------|
| 3.5.1a | Railway service config | `backend/Dockerfile`, optional `railway.toml` / `Procfile` | 0.2 |
| 3.5.1b | PostgreSQL for production | `DATABASE_URL` on Railway; SQLAlchemy works with Postgres URL | 1.1.2 |
| 3.5.1c | Railway env vars | `GROQ_API_KEY`, `CORS_ORIGINS`, `SESSION_TTL_HOURS`, `MAX_UPLOAD_SIZE_MB` | 3.5.1b |
| 3.5.1d | Persistent storage (if SQLite) | Railway volume mounted at `/app/data` | 3.5.1a |
| 3.5.1e | Health check | Railway uses `GET /api/v1/health` | 1.1 |
| 3.5.1f | Public API URL | Note Railway-assigned domain (e.g. `*.up.railway.app`) | 3.5.1e |

**Railway setup checklist:**
- [ ] New project → deploy from GitHub repo, **root directory** = `backend/`
- [ ] Add **PostgreSQL** plugin (recommended) or volume for SQLite
- [ ] Set all secrets in Railway dashboard (not committed)
- [ ] Confirm `/api/v1/health` returns 200 after deploy

#### 3.5.2 — Frontend on Vercel

| # | Task | Files / Modules | Depends On |
|---|------|-----------------|------------|
| 3.5.2a | Vercel project config | `frontend/vercel.json` (SPA rewrites for React Router) | 1.6 |
| 3.5.2b | Build settings | Framework: Vite; output: `dist`; install/build from `frontend/` | 3.5.2a |
| 3.5.2c | `VITE_API_URL` | Vercel env → Railway public API URL (Production + Preview) | 3.5.1f |
| 3.5.2d | SPA routing | Rewrite all non-asset paths to `index.html` (`/analysis/:id`) | 3.5.2a |

**Vercel setup checklist:**
- [ ] Import repo → set **Root Directory** = `frontend/`
- [ ] Build command: `npm run build`; output directory: `dist`
- [ ] Set `VITE_API_URL=https://<your-railway-api-domain>` in Vercel env
- [ ] After deploy, update Railway `CORS_ORIGINS` with Vercel production URL

#### 3.5.3 — Integration & Documentation

| # | Task | Files / Modules | Depends On |
|---|------|-----------------|------------|
| 3.5.3a | CORS + HTTPS | Railway `CORS_ORIGINS` includes Vercel domain(s) | 3.5.1c, 3.5.2c |
| 3.5.3b | README deploy section | Railway + Vercel steps, env tables, demo URLs | — |
| 3.5.3c | `.env.example` updates | Document production vars (`VITE_API_URL`, Postgres URL) | 0.5 |
| 3.5.3d | E2E smoke test | Upload on Vercel URL → dashboard → export | All phases |

**Acceptance criteria:**
- [ ] Vercel demo URL loads SPA and calls Railway API successfully (no CORS errors)
- [ ] Upload CSV on production → analysis dashboard completes
- [ ] README documents both deploy paths; evaluator can run locally in < 15 minutes
- [ ] E2E test passes in CI or documented manual script against production URLs

**Out of scope for Phase 3 deploy:**
- Single-container monolith (API serving static frontend) — replaced by Railway + Vercel split
- Docker Compose in production — dev-only
...
- [ ] Spend summary dashboard with charts
- [ ] ≥ 3 personalized financial insights (≥ 5 with LLM)
- [ ] Downloadable / shareable report
- [ ] Privacy: TTL, delete endpoint, no persistent raw files
- [ ] **Deployed:** Vercel frontend URL + Railway API URL documented in README
- [ ] Production upload works end-to-end (Vercel → Railway → Postgres/SQLite)
