...
├── filename, file\_type, bank\_hint (optional)

├── status: pending | parsing | processing | ready | failed

├── uploaded\_at, expires\_at (TTL for privacy)
...
├── description\_raw # original text from statement

├── description\_clean # normalized merchant/description
...
├── category\_confidence # 0.0–1.0

├── is\_recurring # boolean

├── recurring\_group\_id # links related recurring txns
...
"description\_raw": "UPI-SWIGGY-BANGALORE-123456",
...
● Descriptions → trimmed; preserve description\_raw for audit

● Duplicates → detect via (date, amount, description\_raw) hash within a session
...
def can\_parse(self, file: UploadFile) -> bool: ...
...
Output: description\_clean, optional merchant, payment\_mode.
...
● Batch transactions (e.g. 20–50 per call) with description\_clean, amount, type
...
1\. Group debits by similar description\_clean (fuzzy match, Levenshtein or token overlap)
...
Output: RecurringGroup records + is\_recurring flags on transactions.
...
PATCH /sessions/{id}/transactions/{txn\_id} User override category (optional)
...
→ 202 { session\_id, status: "processing" }
...
● transactions(session\_id, date)

● transactions(session\_id, category)

● recurring\_groups(session\_id)
...
● Structured logs: session\_id, stage, duration\_ms, row\_count
...
This architecture prioritizes a complete, demonstrable workflow. Depth can be added incrementally to parsers and

categorization without changing the overall shape of the system.
