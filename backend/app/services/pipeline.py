...
def run_pipeline(
    ...
    file_content: bytes,
    filename: str,
    column_mapping: object | None = None,
    header_row: int = 0,
) -> UploadSession:
    ...
    if column_mapping:
        from app.parsers.csv_parser import parse_csv_with_mapping
        from app.parsers.base import ParseResult
        try:
            txns = parse_csv_with_mapping(file_content, column_mapping, header_row)
            parse_result = ParseResult(transactions=txns)
            parser = "custom_mapping"
        except Exception as e:
            _set_status(db, session, SessionStatus.FAILED, f"Failed to parse with custom mapping: {str(e)}")
            return session
    else:
