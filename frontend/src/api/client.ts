...
export async function previewCsv(file: File): Promise<CsvPreview> {
  const form = new FormData();
  form.append("file", file);
  const response = await fetch(`${API_URL}/api/v1/upload/preview`, { method: "POST", body: form });
  return handleResponse(response);
}

export async function getBankPresets(): Promise<BankPreset[]> {
  const response = await fetch(`${API_URL}/api/v1/upload/bank-presets`);
  return handleResponse(response);
}

export async function uploadStatementWithMapping(
  file: File,
  ...
  const response = await fetch(`${API_URL}/api/v1/upload/upload-with-mapping`, {
    method: "POST",
    body: form,
  });
  return handleResponse(response);
